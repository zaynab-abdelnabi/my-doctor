...
export const register = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "the new user have been added successfully" });
    } catch (e) {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (authSuccess) {
            const token = jsonwebtoken.sign({ id: user.id, name: user.name, email: user.email, password: user.password }, process.env.JWT_SECRET);
